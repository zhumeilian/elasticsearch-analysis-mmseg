...
import redis.AddTermRedisPubSub;
import redis.RedisPoolBuilder;
import redis.RedisUtils;
import redis.clients.jedis.Jedis;
import redis.clients.jedis.JedisPool;

public class MMsegElasticConfigurator {
    public static ESLogger logger = Loggers.getLogger("mmseg-tokenizer");
    ...
    private static void initRedis(final Settings settings) {
		...
		new Thread(new  Runnable() {
			...
			public void run() {
				...
				String channel = settings.get("redis.channel","mmseg_term");
    ...
    public static void setLoaded(boolean loaded) {
    	MMsegElasticConfigurator.loaded = loaded;
    }
